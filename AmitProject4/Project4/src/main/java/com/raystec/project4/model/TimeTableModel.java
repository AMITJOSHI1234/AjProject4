...
public class TimeTableModel {
	...
	public void update(TimeTableBean bean) throws ApplicationException, DublicateRecordException {
		...
		if (bean1 != null || bean2 != null) {
			throw new DublicateRecordException("TimeTable Already Exsist");
		...
		try {
			...
			PreparedStatement pstmt = conn.prepareStatement(
					"update ST_timetable set course_name=?,course_id=?,subject_name=?,subject_id=?,exam_date=?,exam_time=?,semister=?,created_by=?,modified_by=?,created_datetime=?,modified_datetime=? where ID=?");
	...
	public TimeTableBean checkBycss(long CourseId, long SubjectId, String semester) throws ApplicationException {
		...
		StringBuffer sql = new StringBuffer(
				"SELECT * FROM ST_TIMETABLE WHERE Course_ID=? AND Subject_ID=? AND Semister=?");
	...
	public TimeTableBean checkBycds(long CourseId, String Semester, Date examDate) throws ApplicationException {
		StringBuffer sql = new StringBuffer(
				"SELECT * FROM ST_TIMETABLE WHERE Course_Id=? AND semister=? AND Exam_Date=?");
